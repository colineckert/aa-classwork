// O(wh) time | O(wh) space - width and height of matrix
// can also be described as O(n) time & space - all nodes in matrix
  ...
  const sizes = [];
	...
	for (let i = 0; i < matrix.length; i++) {
		for (let j = 0; j < matrix[i].length; j++) {
			...
			dfsTraverseNode(i, j, matrix, visited, sizes);
		}
	}
...
function dfsTraverseNode(i, j, matrix, visited, sizes) {
